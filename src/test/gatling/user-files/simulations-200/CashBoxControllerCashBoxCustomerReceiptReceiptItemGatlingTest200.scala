 ...
 */
class CashBoxControllerCashBoxCustomerReceiptReceiptItemGatlingTest200 extends Simulation {
    ...
    setUp(
        users.inject(rampUsers(Integer.getInteger("users", 200)) over (Integer.getInteger("ramp", 1) minutes))
    ).protocols(httpConf)
}
