 ...
 */
class CashBoxControllerCashBoxCustomerReceiptReceiptItemGatlingTest2000 extends Simulation {
    ...
    setUp(
        users.inject(rampUsers(Integer.getInteger("users", 2000)) over (Integer.getInteger("ramp", 2) minutes))
    ).protocols(httpConf)
}
