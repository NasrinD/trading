...
public class InventoryResource {
    ...
    public List<Inventory> getAllInventories(@RequestParam(required = false) String filter) {
        ...
        if ("cashdeskapplication-is-null".equals(filter)) {
            log.debug("REST request to get all Inventorys where cashDeskApplication is null");
            return StreamSupport
                .stream(inventoryRepository.findAll().spliterator(), false)
                .filter(inventory -> inventory.getCashDeskApplication() == null)
                .collect(Collectors.toList());
        }
